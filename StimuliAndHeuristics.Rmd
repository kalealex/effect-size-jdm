...
budget <- 1
min_bet <- 0.05
possible_bets <- seq(from=min_bet, to=budget, by=0.05) 
...
We will vary the mean and standard deviation of the distribution users are judging so that we measure different levels of uncertainty (e.g., low and high) and different odds of victory for team A (e.g., 1:1, 2:1, 4:1, 8:1, 16:1). The point of these conditions is that the mean margin of victory for team A depends on both the variance and the odds of victory for team A, such that the location of the mean alone should provide a poor cue for the task.
...
conds_df$mean <- -(conds_df$sd * qnorm(1 - conds_df$odds_of_victory)) # see for derivation https://www.johndcook.com/quantiles_parameters.pdf
...
conds_df %>% ggplot(aes(x=mean, y="Model Prediction")) +
  ...
  labs(
    ...
    x="Margin of Victory",
    caption="Odds of Victory * Levels of Uncertainty"
  ) +
...
$$Perceived Pr(A_{win}) \propto 50 + 50 * (\max{\mu(A - B)} - \mu(A - B))$$
This predicts that a user will underestimate the probability that team A wins as the mean difference between scores for teams A and B is more negative. This should result in excessively conservative betting for extreme odds.
...
conds_df %>% ggplot(aes(y=mean, x="Model Prediction")) +
  geom_errorbar(aes(ymin=mean-qnorm(0.95)*sd, ymax=mean+qnorm(0.95)*sd, width=0)) +
  coord_flip() +
  theme_bw() +
  labs(
...
When relying on intervals to make judgments about CLES, I predict that users will use the proportion of the interval which crosses zero as a cue to the probability that team A wins the game.
$$Perceived Pr(A_{win}) \propto \frac{interval > 0}{interval}$$
This predicts that users will overestimate propability of values near the edge of the interval and underestimate the probability of values near the center of the interval. I would need to run a simulation of this strategy to predict the exact pattern of influence on betting. However, it follows that users following this heuristic will bet too much on extreme odds.
...
$$Perceived Pr(A_{win}) \propto \frac{\Sigma(draws > 0)}{\Sigma (draws)}$$
...
Similar to HOPs, I would expect users of quantile dotplots to judge CLES based on the proportion of dots crossing the zero line.
$$Perceived Pr(A_{win}) \propto \frac{\Sigma(dots > 0)}{\Sigma (dots)}$$
...
# means with intervals
conds_df %>% ggplot(aes(y=mean, x="Model Prediction")) +
  geom_pointrange(aes(ymin=mean-qnorm(0.95)*sd, ymax=mean+qnorm(0.95)*sd)) +
  coord_flip() +
  theme_bw() +
  labs(
...
conds_df2$mean_diff <- conds_df2$sd_diff * qnorm(1 - conds_df2$odds_of_victory)
...
# compute the sd of distributions A and B, assumping independent and equal variances
...
When relying on means alone to make judgments about CLES, users have no uncertainty information. As Jessica proposed in the analysis of her 2015 data she shared with me, we should model the perceived reliability of the effect from means alone as a function of the position of the mean. Specifically:
$$Perceived Pr(A > B) \propto 50 + 50 * (\max{(\mu_{A} - \mu_{B})} - (\mu_{A} - \mu_{B}))$$
This predicts that a user will underestimate the probability that team A wins as the mean difference between scores for teams A and B becomes greater. This should result in excessively conservative betting for extreme odds.

Alternatively, the heuristic might be relative to the length of the axis such that:
$$Perceived Pr(A > B) \propto (\mu_{A} - \mu_{B}) / Axis Range$$
...
$$Perceived Pr(A > B) \propto 100 - 50 * \frac{\Sigma(Interval Overlap)}{\Sigma(IntervalLength)}$$
or
$$Perceived Pr(A > B) \propto 100 - 50 * \frac{\Sigma(Interval Overlap)}{Axis Range}$$
These heuristics predict that users will overestimate $Pr(A > B)$ when there is small overlap and underestimate $Pr(A > B)$ when overlap is large. I would need to run a simulation of this strategy to predict the exact pattern of influence on betting. However, it follows that users following this heuristic will bet too much on extreme odds.
...
$$Perceived Pr(A > B) \propto \frac{\Sigma(draws_{A} > draws_{B})}{\Sigma (draws)}$$
Given representative sampling this should be pretty close to the ground truth. Although maybe we can test order effects---and thus differentiate the subitizing heuristic from the ground truth---by manipulating the series of draws and modeling primacy and recency as different weighting functions over the timeseries of draws.
...
I would expect the heuristic in this case to be based on the number of dots that overlap.
$$Perceived Pr(A >B) \propto 100 - 50 * \frac{\Sigma(DotsOverlap)}{\Sigma(Dots)}$$
...
For densities, I expect the heuristic will be based on the proportion of overlap in area, similar to the interval overlap heuristic.
$$Perceived Pr(A >B) \propto 100 - 50 * \frac{\Sigma(AreaOverlap)}{\Sigma(Area)}$$
...
$$Perceived Pr(A > B) \propto 100 - 50 * \frac{\mu_{A} - \mu_{B}}{\Sigma(IntervalLength)}$$
