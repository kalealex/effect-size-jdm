...
# set plot dimensions
dims <- c(770, 462) # pixel dims for gifs
...
for (c in unique(conds_df$condition)) {
  ...
  ggsave(file=fname, plot=plt, width=dims[1]/ppi, height=dims[2]/ppi)
  ...
  plt <- use_df %>% select(-one_of(c("quantiles"))) %>% unnest() %>%
    ...
    geom_point(shape=124, size=means_size) +
    coord_flip() +
    theme_bw() +
    colScale +
    ylim(data_domain[1], data_domain[2]) +
