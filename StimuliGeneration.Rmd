...
for (c in unique(conds_df$condition)) {
  ...
  plt <- use_df %>% ggplot(aes(x=mean, y=reorder(Team, desc(Team)), color=Team)) +
    ...
    labs(
      ...
      y=NULL,
      caption=wrap_label("Dots represent the average score in past games for each team.", 67)
    ) +
    theme(
  ...
  plt <- use_df %>% ggplot(aes(y=mean, x=reorder(Team, desc(Team)), color=Team)) +
    ...
    labs(
      ...
      caption=wrap_label("Intervals represent the span of past scores for 95% of games played by each team.", 67)
    ) +
    theme(
      plot.caption=element_text(size=10, hjust=0)
    )
  ...
  plt <- use_df %>% select(-one_of(c("quantiles"))) %>% unnest() %>%
    ...
    labs(
      ...
      y="Past Scores",
      x=NULL,
      caption=wrap_label("Lines represent past scores in individual games for each team.", 80)
    ) +
    theme(
  ...
  hops <- animate(plt, fps=2.5, width=dims[1], height=dims[2])
  ...
  plt <- use_df %>% select(-one_of(c("draws", "draw_n"))) %>% unnest() %>%
    ...
    labs(
      ...
      caption=wrap_label("Each dot represents 5% of past scores for each team.", 67)
    ) +
    facet_grid(Team ~ .) +
    theme(
  ...
  plt <- use_df %>% select(-one_of(c("draws", "draw_n"))) %>% unnest() %>%
    ...
    labs(
      ...
      x="Past Scores",
      y="Proportion of Past Scores",
      caption=wrap_label("The height of the line represents the proportion of past scores for each team.", 67)
    ) + 
    theme(
      plot.caption=element_text(size=10, hjust=0)
    )
  ...
  plt <- use_df %>% ggplot(aes(y=mean, x=reorder(Team, desc(Team)), color=Team)) +
    ...
    labs(
      ...
      y="Past Scores",
      x=NULL,
      caption=wrap_label("Dots represent the average score in past games for each team, and intervals represent the span of past scores for 95% of games.", 67)
    ) +
    theme(
      plot.caption=element_text(size=10, hjust=0)
    )
  ...
  plt <- use_df %>% select(-one_of(c("quantiles"))) %>% unnest() %>%
    ...
    labs(
      ...
      y="Past Scores",
      x=NULL,
      caption=wrap_label("Dots represent the average score in past games for each team, and lines represent past scores in individual games.", 80)
    ) +
    theme(
      plot.title=element_text(size=16, hjust=0),
      plot.caption=element_text(size=11, hjust=0)
