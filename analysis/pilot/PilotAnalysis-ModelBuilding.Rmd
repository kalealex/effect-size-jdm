...
responses_df <- responses_df %>%
  ...
  mutate(trial_dur = ifelse(trial_dur < 0, 0, trial_dur)) # avoid negative trial durations from faulty reconstruction (only one case)
...
```{r}
# fit model (TODO: post issue for compose_data not handling nested data)
# m <- sampling(stan_model, data=compose_data(model_df), control=list(adapt_delta=0.99))
```
