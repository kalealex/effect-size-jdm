...
```{r}
# trace plots
traceplot(m.hops.multi_heuristic_hier, pars = c('mu_lo_heuristic','sigma_lo_heuristic','sigma'))
```
...
```{r}
# pairs plot
pairs(m.hops.multi_heuristic_hier, pars = c('mu_lo_heuristic','sigma_lo_heuristic','sigma'))
```
...
```{r}
# plot transformed posterior for probability of each heuristic
m.hops.multi_heuristic_hier %>%
  spread_draws(mu_lo_heuristic[heuristic], sigma_lo_heuristic) %>%
  mutate(
    lo_heuristic = rnorm(n(), mu_lo_heuristic, sigma_lo_heuristic) # TODO need to gather these into vectos and apply softmax function
  ) %>%
  ggplot(aes(x=ground_truth, y=y_rep)) +
