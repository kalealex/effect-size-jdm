...
fake_worker_params_df <- model_df_hops %>%
  select(worker_id) %>%
  distinct(worker_id) %>%
  mutate(mu_lo_heuristic_worker = list(MASS::mvrnorm(1, qlogis(p_heuristic), Sigma))) %>% # intermediate calculation: draws from multivariate normal
  rowwise() %>%
  mutate(
...
fake_df_hops <- model_df_hops %>%
  ...
  mutate(
    ...
    cles = plogis(lo_cles) # simulated responses from known data generating process
  )
```
...
```{r}
# trace plots
traceplot(m.fake.hops.multi_heuristic_hier, pars = c('mu_lo_heuristic','sigma_scale','sigma_err'))
```
...
```{r}
# pairs plot
pairs(m.fake.hops.multi_heuristic_hier, pars = c('mu_lo_heuristic','sigma_scale','sigma_err'))
```
...
How did we do at recovering the fake values for p_heuristic? Let's plot posterior predictive distributions to find out.
...
Maybe the issues we are having are unique to the HOPs condition, where the heuristics we've identified so far don't seem to describe user behavior.
...
```{r}
# trace plots
traceplot(m.intervals_w_means.multi_heuristic_hier, pars = c('mu_lo_heuristic','sigma_scale','sigma_err'))
```
...
```{r}
# pairs plot
pairs(m.intervals_w_means.multi_heuristic_hier, pars = c('mu_lo_heuristic','sigma_scale','sigma_err'))
```
