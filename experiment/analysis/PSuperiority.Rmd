...
# complete_set <- model_df %>%     # if we want to keep only the participants with complete data sets
...
#   ) %>%
#   filter(dropped_trials == 0) 
# complete_set <- complete_set$worker_id
# model_df <- model_df %>%
#   filter(worker_id %in% complete_set)
...
<!-- ```{r} -->
<!-- # filter to 20 participants per condition for faster model testing -->
<!-- test_set <- model_df %>% -->
<!--   group_by(start_means, condition, worker_id) %>% -->
<!--   summarise() %>% -->
<!--   top_n(10) -->
<!-- test_set <- test_set$worker_id -->
<!-- model_df <- model_df %>% -->
<!--   filter(worker_id %in% test_set) -->
...
<!-- ```{r} -->
<!-- m <- gamlss(lo_p_sup ~ re(fixed = ~ clo_ground_truth*means*sd_diff*condition,  -->
<!--                           random = ~ 1 + clo_ground_truth|worker_id),  -->
<!--             sigma.fo = ~ re(random = ~ 1|worker_id),  -->
