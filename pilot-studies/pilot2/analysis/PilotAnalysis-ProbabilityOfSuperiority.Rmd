...
We need the data in a format where it is prepared for modeling. This means converting both probability of superiority judgments and the ground truth to a logit scale. It also means that we want baseline as a factor rather than a numeric value.
...
Again, this is promising, but it is still very noisy. Also, there are areas of high posterior density in the interals_w_means and means_only conditions where there are no observations. Maybe adding the baseline condition to the model will improve the fit?


##Add a Slope Interaction for Baseline Condition

Similar to how we model the effects of visualization conditions on the slope of the LLO model as interactions with the ground truth, we add an interaction for baseline conditions. This gives us a three way interaction.

```{r}
# update the llo model of p_sup responses to include an interaction
m.vis.base.wrkr.llo_p_sup <- update(m.llo_p_sup, 
                         formula = lo_p_sup ~ (1 + lo_ground_truth|worker_id) + lo_ground_truth:condition:baseline,
                         newdata = model_df_llo,
                         file = "model-fits/llo_mdl_vis_base_wrkr")
```
...
```{r}
# trace plots
plot(m.vis.base.wrkr.llo_p_sup)
```
...
```{r}
# pairs plot
pairs(m.vis.base.wrkr.llo_p_sup)
```
...
```{r}
# model summary
print(m.vis.base.wrkr.llo_p_sup)
```
...
# posterior predictive check
model_df_llo %>%
  select(lo_ground_truth, condition, baseline, worker_id) %>%
  add_predicted_draws(m.vis.base.wrkr.llo_p_sup, prediction = "lo_p_sup", seed = 1234) %>%
  mutate(post_p_sup = plogis(lo_p_sup)) %>%
  ggplot(aes(x = post_p_sup)) +
  geom_density(fill = "black", size = 0) +
  scale_y_continuous(NULL, breaks = NULL) +
...
```{r}
# use posterior samples to define distributions for the slope in each visualization condition
posterior_samples(m.vis.base.wrkr.llo_p_sup) %>%
  transmute(slope_HOPs = `b_lo_ground_truth:conditionHOPs:baseline0.5` + `b_lo_ground_truth:conditionHOPs:baseline0.85`,
            slope_intervals_w_means = `b_lo_ground_truth:conditionintervals_w_means:baseline0.5` + `b_lo_ground_truth:conditionintervals_w_means:baseline0.85`,
            slope_means_only = `b_lo_ground_truth:conditionmeans_only:baseline0.5` + `b_lo_ground_truth:conditionmeans_only:baseline0.85`) %>%
  gather(key, value) %>%
  ggplot(aes(x = value, group = key, color = key, fill = key)) +
  geom_density(alpha = 0.35) +
  scale_fill_brewer(type = "qual", palette = 1) +
...
Let's take a look at some of the estimated linear models per visualization * baseline condition.

```{r}
# this time we'll adopt functions from the tidybayes package to make plotting posterior predictions easier
model_df_llo %>%
  group_by(condition, baseline, worker_id) %>%
  data_grid(lo_ground_truth = seq_range(lo_ground_truth, n = 51)) %>%
  add_predicted_draws(m.vis.base.wrkr.llo_p_sup) %>%
  ggplot(aes(x = lo_ground_truth, y = lo_p_sup, color = condition, fill = condition)) +
  geom_abline(intercept = 0, slope = 1, size = 1, alpha = .3, color = "red", linetype = "dashed") + # ground truth
  stat_lineribbon(aes(y = .prediction), .width = c(.95, .80, .50), alpha = .25) +
  geom_point(data = model_df_llo) +
...
Adding an interaction for the baseline condition doesn't really fix the problem that our posterior predictions have high density where there are few responses. However, it does seem like probability of superiority judgments may be slightly more distorted when the baseline is 0.5.

Let's check whether adding parameters to account for baseline is worth it insofar as the added parameters contribute more to predictive validity than they contribute to overfitting. We'll determine this by comparing the models with and without parameters for baseline according to the widely applicable information criterion (WAIC). Lower values of WAIC indicate a better fitting model.

```{r}
waic(m.vis.wrkr.llo_p_sup, m.vis.base.wrkr.llo_p_sup)
```

The fact that WAIC values for the two models are approximately equal suggests that we don't get much improvement in model fit from adding the parameters for baseline condition to this model. This makes sense as the baseline conditions are more likely to impact decisions than probability judgments.
...
process_df <- responses_df %>% rowwise() %>% 
  ...
  select(worker_id, baseline, ground_truth, process, process_est) %>%
...
fake_df_llo_mix <- model_df_llo %>%
  ...
  left_join(process_df, by = c("worker_id", "baseline", "ground_truth", "process")) %>% # add process estimates to model_df
