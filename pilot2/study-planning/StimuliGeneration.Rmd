...
conds_df <- data.frame(
  "p_superiority" = rep(p_sup, length(base) * length(K)),
  "baseline" = rep(sort(rep(base, length(p_sup))), length(K)),
  "K" = sort(rep(K, length(p_sup) * length(base))))
  ...
  if (all(use_df$frame=="gain")) { # stimuli for gain framing trials
    ...
    plt <- use_df %>% ggplot(aes(x = mean, y = reorder(Machine, desc(Machine)), color = Machine)) +
      ...
      labs(
        ...
        x = "Average Number of Widgets Produced in Previous Years (in Millions)",
        y = NULL,
        caption = wrap_label("Dots represent the average number of widgets produced (in millions) by each machine in previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 500, color = "gray", linetype = "longdash") # contract threshold
    ...
    plt <- use_df %>% ggplot(aes(y = mean, x = reorder(Machine, desc(Machine)), color = Machine)) +
      ...
      labs(
        title = "Millions of Widgets Produced in Previous Years",
        x = NULL,
        y = "Range Containing Number of Widgets Produced in 95% of Previous Years (in Millions)",
        caption = wrap_label("Intervals represent the span of widgets produced (in millions) by each machine in 95% of previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 500, color = "gray", linetype = "longdash") # contract threshold
    ...
    plt <- use_df %>% select(-one_of(c("quantiles"))) %>% unnest() %>%
      ...
      labs(
        title = "Millions of Widgets Produced in Previous Years",
        y = "Number of Widgets Produced in Previous Years (in Millions)",
        x = NULL,
        caption = wrap_label("Lines represent the number of widgets produced (in millions) by each machine in individual previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 500, color = "gray", linetype = "longdash") + # contract threshold
    ...
    plt <- use_df %>% select(-one_of(c("draws", "draw_n"))) %>% unnest() %>%
      ...
      geom_dotplot(binwidth = 7.5, binaxis = "x", dotsize = 0.80, stackratio = 1.20) +
      theme_bw() +
      scale_fill_brewer(palette = "Set1") +
      xlim(data_domain_gain[1], data_domain_gain[2]) +
      labs(
        title = "Millions of Widgets Produced in Previous Years",
        x = "Number of Widgets Produced in Previous Years (in Millions)",
        y = "Proportion of Widgets Produced",
        caption = wrap_label("Each dot represents 5% of the number widgets produced (in millions) by each machine in previous years.", char_before_wrap)) +
      # facet_grid(Machine ~ .) +
      theme(
    ...
    # densities
    plt <- use_df %>% select(-one_of(c("draws", "draw_n"))) %>% unnest() %>%
      ggplot(aes(x = quantiles, color = Machine)) +
      geom_density() +
      theme_bw() +
      colScale +
      xlim(data_domain_gain[1], data_domain_gain[2]) +
      labs(
        ...
        x = "Number of Widgets Produced in Previous Years (in Millions)",
        y = "Proportion of Widgets Produced",
        caption = wrap_label("The height of the line represents the proportion of the number widgets produced (in millions) by each machine in previous years.", char_before_wrap)) +
      # facet_grid(Machine ~ .) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
    ...
    plt <- use_df %>% ggplot(aes(y = mean, x = reorder(Machine, desc(Machine)), color = Machine)) +
      ...
      labs(
        ...
        y = "Number of Widgets Produced in Previous Years (in Millions)",
        x = NULL,
        caption = wrap_label("Dots represent the average number of widgets produced (in millions) by each machine in previous years, and intervals represent the span of widgets produced in 95% of previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 500, color = "gray", linetype = "longdash") # contract threshold
    ...
    plt <- use_df %>% select(-one_of(c("quantiles"))) %>% unnest() %>%
      ...
      labs(
        ...
        y = "Number of Widgets Produced in Previous Years (in Millions)",
        x = NULL,
        caption = wrap_label("Dots represent the average number of widgets produced (in millions) by each machine in previous years, and lines represent widgets produced in individual previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 500, color = "gray", linetype = "longdash") + # contract threshold
    ...
    plt <- use_df %>% ggplot(aes(x = mean, y = reorder(Machine, desc(Machine)), color = Machine)) +
      ...
      labs(
        ...
        x = "Average Number of Defective Widgets per Million Produced in Previous Years",
        y = NULL,
        caption = wrap_label("Dots represent the average number of defective widgets per million produced by each machine in previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 75, color = "gray", linetype = "longdash") # contract threshold
    fname <- paste("stimuli/means_only-", c,".svg", sep = "")
    ggsave(file = fname, plot = plt, width = dims[1], height = dims[2])
    ...
    plt <- use_df %>% ggplot(aes(y = mean, x = reorder(Machine, desc(Machine)), color = Machine)) +
      ...
      labs(
        title = "Defective Widgets per Million Produced in Previous Years",
        x = NULL,
        y = "Range Containing Number of Defective Widgets per Million Produced in 95% of Previous Years",
        caption = wrap_label("Intervals represent the span of defective widgets per million produced by each machine in 95% of previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 75, color = "gray", linetype = "longdash") # contract threshold
    ...
    plt <- use_df %>% select(-one_of(c("quantiles"))) %>% unnest() %>%
      ...
      labs(
        title = "Defective Widgets per Million Produced in Previous Years",
        y = "Number of Defective Widgets per Million Produced in Previous Years",
        x = NULL,
        caption = wrap_label("Lines represent the number of defective widgets per million produced by each machine in individual previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 75, color = "gray", linetype = "longdash") + # contract threshold
    ...
    plt <- use_df %>% select(-one_of(c("draws", "draw_n"))) %>% unnest() %>%
      ggplot(aes(x = quantiles, fill = Machine)) +
      geom_dotplot(binwidth = 7.5, binaxis = "x", dotsize = 0.8, stackratio = 1.20) +
      theme_bw() +
      scale_fill_brewer(palette = "Set1") +
      xlim(data_domain_loss[1], data_domain_loss[2]) +
      labs(
        title = "Defective Widgets per Million Produced in Previous Years",
        x = "Number of Defective Widgets per Million Produced in Previous Years",
        y = "Proportion of Defective Widgets Produced",
        caption = wrap_label("Each dot represents 5% of the defective widgets per million produced by each machine in previous years.", char_before_wrap)) +
      # facet_grid(Machine ~ .) +
      theme(
        strip.background = element_blank(),
        strip.text.x = element_blank(),
    ...
    plt <- use_df %>% select(-one_of(c("draws", "draw_n"))) %>% unnest() %>%
      ...
      labs(
        ...
        x = "Number of Defective Widgets per Million Produced in Previous Years",
        y = "Proportion of Defective Widgets Produced",
        caption = wrap_label("The height of the line represents the proportion of the defective widgets per million produced by each machine in previous years.", char_before_wrap)) +
      # facet_grid(Machine ~ .) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
    ...
    plt <- use_df %>% ggplot(aes(y = mean, x = reorder(Machine, desc(Machine)), color = Machine)) +
      ...
      labs(
        ...
        y = "Number of Defective Widgets per Million Produced in Previous Years",
        x = NULL,
        caption = wrap_label("Dots represent the average number of defective widgets per million produced by each machine in previous years, and intervals represent the span of defective widgets per million produced in 95% of previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 75, color = "gray", linetype = "longdash") # contract threshold
    ...
    plt <- use_df %>% select(-one_of(c("quantiles"))) %>% unnest() %>%
      ...
      labs(
        ...
        y = "Number of Defective Widgets per Million Produced in Previous Years",
        x = NULL,
        caption = wrap_label("Dots represent the average number of defective widgets per million produced by each machine in previous years, and lines represent defective widgets per million produced in individual previous years.", char_before_wrap)) +
      theme(
        plot.title = element_text(size = title_size),
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 75, color = "gray", linetype = "longdash") + # contract threshold
...
```{r}
# quantile dotplots with opaque dots
use_df %>% select(-one_of(c("draws", "draw_n"))) %>% unnest() %>%
      ggplot(aes(x = quantiles, y = "Proportion of Widgets Produced", fill = Machine)) +
      geom_dotplot(alpha = 0.75, binwidth = 7.5, binaxis = "x", dotsize = 0.90, stackratio = 1.10) +
      theme_bw() +
      colScale +
      xlim(data_domain_gain[1], data_domain_gain[2]) +
      labs(
      ...
      theme(
        ...
        plot.caption = element_text(size = caption_size, hjust = 0)) +
      geom_vline(xintercept = 500, color = "gray", linetype = "longdash") + # contract threshold
      geom_text(aes(x = 500, y = 1, label = "Above this threshold,\nyou gain the contract."), color = "gray", hjust=0, vjust=0, nudge_x = 2, nudge_y = .4) +
      facet_grid(Machine ~ .)
```
