---
title: "Pilot Analysis: Censored Linear Log Odds Model of Probability of Superiority for Gain Frame Trials Only"
...
In this document, we build a censored linear log odds model of probability of superiority judgments. 
...
Because we constrained the response scale to 50-100, we are going to try using a censored model of probability of superiority judgments. The censored modeling approach can no longer support the mixture we were using before.
...
model_df_llo <- responses_df %>%
  ...
  mutate( 
    # censor probability of superiority beyond boundaries
    p_sup_censored = if_else(p_superiority < 50.5, 50.5, as.numeric(p_superiority)),   # censor responses less than 50.5%
    p_sup_censored = if_else(p_sup_censored > 99.5, 99.5, p_sup_censored), # censor responses greater than 99.5%
    # code censoring
    censored = if_else(p_sup_censored == 50.5,
                       "left",
                       if_else(p_sup_censored == 99.5,
                                "right",
                                "none")),
    censored = as.factor(censored), # to avoid modeling error
    # apply logit function to p_sup judgments and ground truth
    lo_p_sup = qlogis(p_sup_censored / 100),
...
Let's look at our prior predictive distribution. For this intercept model, it should be approximately flat with a mode at 50% due to censoring.
...
model_df_llo %>%
  ...
  mutate(
    ...
    # replicate censoring
    post_p_sup = if_else(post_p_sup < 0.505, 
                         0.505,
                         if_else(post_p_sup > 0.995,
                                 0.995,
                                 post_p_sup))
    ) %>%
...
```{r}
# get_prior(data = model_df_llo, family = "gaussian", formula = lo_p_sup | cens(censored) ~ 0 + intercept + lo_ground_truth)

# simple LLO model
prior.llo_p_sup <- brm(data = model_df_llo, family = "gaussian",
              lo_p_sup | cens(censored) ~ 0 + intercept + lo_ground_truth, # non-centered intercept
...
Let's look at our prior predictive distribution. For this linear model, we should see a mode at 50% due to censoring and a mode near 100% due our skewed sampling of the ground truth. 
...
m.llo_p_sup <- brm(data = model_df_llo, family = "gaussian",
              lo_p_sup | cens(censored) ~ 0 + intercept + lo_ground_truth,
              prior = c(prior(normal(1, 0.5), class = b),
                        prior(normal(0, 0.02), class = b, coef = intercept),
                        prior(normal(0, 0.5), class = sigma)),
              sample_prior = TRUE,
              iter = 3000, warmup = 500, chains = 2, cores = 2,
              file = "model-fits/llo_mdl_cens")
```
...
Before we fit the model to our data, let's check that our priors seem reasonable. We are adding hyperpriors for the standard deviation of slopes and intercepts per worker, the correlation between them, and residual variation (i.e., sigma) per worker. We'll constrain the variability of intercepts to a narrow range as before, and we'll also narrow the priors on the variability of slopes and sigma since we are now attributing variability to more sources and we want to avoid overdispersion.

```{r}
# get_prior(data = model_df_llo, family = "gaussian", formula = bf(lo_p_sup | cens(censored) ~  0 + intercept + (lo_ground_truth|worker_id) + lo_ground_truth, sigma ~ (1|worker_id)))

# hierarchical LLO model
prior.wrkr.llo_p_sup <- brm(data = model_df_llo, family = "gaussian",
                            formula = bf(lo_p_sup | cens(censored) ~  0 + intercept + (lo_ground_truth|worker_id) + lo_ground_truth, # non-centered intercept
                                         sigma ~ (1|worker_id)),
                            prior = c(prior(normal(1, 0.5), class = b),
                                      prior(normal(0, 0.02), class = b, coef = intercept),
...
# prior predictive check
model_df_llo %>%
  select(censored, lo_ground_truth, worker_id) %>%
...
m.wrkr.llo_p_sup <- brm(data = model_df_llo, family = "gaussian",
                        ...
                        file = "model-fits/llo_mdl_cens-wrkr")

# m.wrkr.llo_p_sup <- brm(data = model_df_llo, family = "gaussian",
#               lo_p_sup | cens(censored) ~  0 + intercept + (lo_ground_truth|worker_id) + lo_ground_truth, # non-centered intercept
#               prior = c(prior(normal(1, 0.5), class = b),
#                         prior(normal(0, 0.1), class = b, coef = intercept),
#                         prior(normal(0, 0.1), class = sd, group = worker_id, coef = Intercept),
#                         prior(normal(0, 0.5), class = sd, group = worker_id, coef = lo_ground_truth),
#                         prior(normal(0, 0.5), class = sigma)),
#               iter = 3000, warmup = 500, chains = 2, cores = 2,
#               file = "model-fits/llo_mdl_cens-wrkr")
```
...
```{r}
model_df_llo %>%
  group_by(censored, lo_ground_truth, worker_id) %>%
...
At an individual level, predictions look okay, but in some cases, a few outlier responses are causing sigma to blow up. This may be an artifact of censoring responses. Overall, our model predictions are still not matching the density of our data. We can try adding visualization condition as a predictor, but I doubt that will resolve the issues we are seeing with the censored model.
...
prior.wrkr.vis.llo_p_sup <- brm(data = model_df_llo, family = "gaussian",
                            formula = bf(lo_p_sup | cens(censored) ~  0 + intercept + (lo_ground_truth|worker_id) + lo_ground_truth:condition,
...
# prior predictive check
model_df_llo %>%
  select(censored, lo_ground_truth, worker_id, condition) %>%
...
# m.wrkr.vis.llo_p_sup <- brm(data = model_df_llo, family = "gaussian",
#                             lo_p_sup | cens(censored) ~  0 + intercept + (lo_ground_truth|worker_id) + lo_ground_truth:condition, # non-centered intercept
#                                               prior = c(prior(normal(1, 0.5), class = b),
#                                                         prior(normal(0, 0.1), class = b, coef = intercept),
#                                                         prior(normal(0, 0.1), class = sd, group = worker_id, coef = Intercept),
#                                                         prior(normal(0, 0.5), class = sd, group = worker_id, coef = lo_ground_truth),
...
```{r}
model_df_llo %>%
  group_by(censored, lo_ground_truth, worker_id, condition) %>%
...
This seems fairly reasonable, but our posterior predictive desity is still overestimating the number of responses at 50% and 100% suggesting over-dispersion.
