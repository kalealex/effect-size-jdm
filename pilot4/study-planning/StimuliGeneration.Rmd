...
intervals_w_means <- function(df, data_domain, title, x_label, caption, decision_threshold, threshold_label, label_mean = FALSE) {
  ...
  if (label_mean) {
    ...
    plt <- plt + 
      geom_segment(x = 2.1, xend = 2.1, y = mean_with, yend = 75, size = 0.5, linetype = "dotted", color = "black") +
      annotate("text", x = 2.1, y = 73, label = "Average predicted score\nwith the new player", hjust = 1, vjust = 0.5) +
      geom_segment(x = 1.1, xend = 1.1, y = mean_without, yend = 75, size = 0.5, linetype = "dotted", color = "black") +
      annotate("text", x = 1.1, y = 73, label = "Average predicted score\nwithout the new player", hjust = 1, vjust = 0.5)
  }
...
quantile_dotplots <- function(df, n_dots, data_domain, title, x_label, caption, decision_threshold, threshold_label) {
  plt <- df %>%
      ...
      stat_dist_dotsh(quantiles = n_dots, binwidth = 5, dotsize = .8, stackratio = 1.2, alpha = opacity, color = NA) +
      theme_bw() +
...
quantile_dotplots_w_means <- function(df, n_dots, data_domain, title, x_label, caption, decision_threshold, threshold_label, label_mean = FALSE) {
  ...
  if (label_mean) {
    ...
    plt <- plt + 
      geom_segment(y = 2.6, yend = 2.6, x = mean_with, xend = 75, size = 0.5, linetype = "dotted", color = "black") +
      annotate("text", y = 2.6, x = 73, label = "Average predicted score\nwith the new player", hjust = 1, vjust = 0.5) +
      geom_segment(y = 1.6, yend = 1.6, x = mean_without, xend = 75, size = 0.5, linetype = "dotted", color = "black") +
      annotate("text", y = 1.6, x = 73, label = "Average predicted score\nwithout the new player", hjust = 1, vjust = 0.5)
  }
