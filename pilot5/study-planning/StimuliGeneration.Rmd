...
quantile_dotplots <- function(df, n_dots, data_domain, title, x_label, caption, decision_threshold, threshold_label) {
  plt <- df %>%
      ...
      annotate("text", x = (decision_threshold + 2), y = "Without the New Player", label = threshold_label, hjust = 0, vjust = -5.4) +
...
quantile_dotplots_w_means <- function(df, n_dots, data_domain, title, x_label, caption, decision_threshold, threshold_label, label_mean = FALSE) {
  plt <- df %>%
      ...
      geom_vline(xintercept = decision_threshold, linetype = "longdash") + # award threshold
      annotate("text", x = (decision_threshold + 2), y = "Without the New Player", label = threshold_label, hjust = 0, vjust = -5.4) +
      # stat_dist_dotsh(quantiles = n_dots, binwidth = 1.65 + 2.5 * unique(df$sd_diff) / 15, dotsize = .67, stackratio = 1.2, alpha = opacity, color = NA) +
      # stat_dist_dotsh(quantiles = n_dots, binwidth = 1.65 + 2.35 * unique(df$sd_diff) / 15, dotsize = .8, stackratio = 1.25, alpha = opacity, color = NA) +
      stat_dist_dotsh(quantiles = n_dots, binwidth = 2.115 + 2.885 * unique(df$sd_diff) / 15, scale = 0.5, dotsize = .8, stackratio = 1.25, alpha = opacity, color = NA) +
      ...
      geom_point(aes(x = mean, color = Team), shape = 124, size = means_size_slab, show.legend = FALSE, position = position_nudge(y = 0.415)) + # add means
      theme_bw() +
      scale_fill_brewer(palette = "Set1") +
      scale_color_brewer(palette = "Set1") +
      coord_cartesian(
  ...
  if (label_mean) {
    ...
    plt <- plt + 
      geom_segment(y = 2.8, yend = 2.8, x = mean_with, xend = 95, size = 0.5, linetype = "dotted", color = "black") +
      annotate("text", y = 2.8, x = 94, label = "Average predicted score\nwith the new player", hjust = 1, vjust = 0.5) +
      geom_segment(y = 1.8, yend = 1.8, x = mean_without, xend = 95, size = 0.5, linetype = "dotted", color = "black") +
      annotate("text", y = 1.8, x = 94, label = "Average predicted score\nwithout the new player", hjust = 1, vjust = 0.5)
  }
...
densities <- function(df, data_domain, title, x_label, caption, decision_threshold, threshold_label) {
  plt <- df %>% 
      ...
      geom_vline(xintercept = decision_threshold, linetype = "longdash") + # award threshold
      annotate("text", x = (decision_threshold + 2), y = "Without the New Player", label = threshold_label, hjust = 0, vjust = -5.4) +
      stat_dist_slabh(alpha = opacity, scale = 0.8) +
      theme_bw() +
      scale_fill_brewer(palette = "Set1") +
      coord_cartesian(
        xlim = data_domain,
